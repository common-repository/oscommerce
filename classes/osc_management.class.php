...
<?php
				for($i = 0 ; $i < $this->db_col_count ; $i++)
				{
					...
					{
?>
					<th scope="col"><a href="?page=osCommerce&paged=<?=$_GET['paged'];?>&osc_action=sorting&order=<?=$order;?>&sort_col=<?=$this->oStruc['colnam'][$i];?>&sort_ord=<?=$ord;?>" title="<?=attribute_escape($this->oStruc['nicenam'][$i]);?>" style="color: #d7d7d7;"><?=$this->oStruc['nicenam'][$i];?></a></th>
<?php
					}
				}
?>			
...
<?php				
					for($j = 0 ; $j < $this->db_col_count ; $j++)
					{
						if($this->oStruc['inlist'][$j] === true)
						{
							...
							{
?>
					<td><strong><?php if(current_user_can('edit_post', $intID)){ ?><a class="row-title" href="?page=osCommerce-edit-form&osc_action=osc_edit&amp;intID=<?=$intID;?>" title="<?=attribute_escape(sprintf(__('Edit "%s"'), $res_arr[$i]->$colnam)); ?>"><?=$res_arr[$i]->$colnam;?></a><?php } else { echo $res_arr[$i]->$colnam; } ?></strong></td>
<?php
							}
							else
							{
?>
					<td><?=$res_arr[$i]->$colnam;?></td>
<?php
							}
						}
					}
...
?>
		<table class="widefat">
        	<tbody>
            	<tr><td><?php _e('No shops found.'); ?></td></tr>
			</tbody>
		</table>
<?php
			}
...
<?php $_SERVER['REQUEST_URI'] = remove_query_arg(array('message'), $_SERVER['REQUEST_URI']);
		...
		<div id="form">
            <fieldset>
                <legend><?php _e('Add Shop','osCommerce'); ?></legend>
                <div class="row"><font color="red" style="font-weight:bold;">*</font> Marks required fields</div>
                <form name="frm_add" action="?page=osCommerce" method="post" onSubmit="javascript: return frm_validate(this);">
                	<div class="row">
                    	<label for="vchShopName"><font color="red">*</font> <?php _e('Shop Name','osCommerce'); ?></label>
                        <input type="text" name="vchShopName" id="vchShopName" value="<?=$_POST['vchShopName'];?>" class="text">
                	</div>
                	<div class="row">
                    	<label for="vchUrl"><font color="red">*</font> <?php _e('Url','osCommerce'); ?></label>
                        <input type="text" name="vchUrl" id="vchUrl" value="<?=$_POST['vchUrl'];?>" class="text">
                	</div>
                	<div class="row">
                    	<label for="vchUsername"><font color="red">*</font> <?php _e('Username','osCommerce'); ?></label>
                        <input type="text" name="vchUsername" id="vchUsername" value="<?=$_POST['vchUsername'];?>" class="text">
                	</div>
                	<div class="row">
                    	<label for="vchPassword"><font color="red">*</font> <?php _e('Password','osCommerce'); ?></label>
                        <input type="text" name="vchPassword" id="vchPassword" value="<?=$_POST['vchPassword'];?>" class="text">
                	</div>
                	<div class="row">
                    	<label for="vchDbName"><font color="red">*</font> <?php _e('DB Name','osCommerce'); ?></label>
                        <input type="text" name="vchDbName" id="vchDbName" value="<?=$_POST['vchDbName'];?>" class="text">
                 	</div>
                 	<div class="row">
                    	<label for="vchHost"><font color="red">*</font> <?php _e('Host','osCommerce'); ?></label>
                        <input type="text" name="vchHost" id="vchHost" value="<?=$_POST['vchHost'];?>" class="text">
                 	</div>
                    <p class="submit">
...
<?php $_SERVER['REQUEST_URI'] = remove_query_arg(array('message'), $_SERVER['REQUEST_URI']);
        ...
        <div id="form">
            <fieldset>
                <legend><?php _e('Edit Shop','osCommerce'); ?></legend>
                <div class="row"><font color="red" style="font-weight:bold;">*</font> Marks required fields</div>
                <form name="frm_edit" method="post" action="?page=osCommerce" onSubmit="javascript: return frm_validate();">
                    <div class="row">
                        <label for="vchShopName"><font color="red">*</font> <?php _e('Shop Name','osCommerce'); ?></label>
                        <input type="text" name="vchShopName" id="vchShopName" value="<?=stripslashes($shop->vchShopName);?>" class="text">
                    </div>
                    <div class="row">
                        <label for="vchUrl"><font color="red">*</font> <?php _e('Url','osCommerce'); ?></label>
                        <input type="text" name="vchUrl" id="vchUrl" value="<?=stripslashes($shop->vchUrl);?>" class="text">
                    </div>
                    <div class="row">
                        <label for="vchUsername"><font color="red">*</font> <?php _e('Username','osCommerce'); ?></label>
                        <input type="text" name="vchUsername" id="vchUsername" value="<?=stripslashes($shop->vchUsername);?>" class="text">
                    </div>
                    <div class="row">
                        <label for="vchPassword"><font color="red">*</font> <?php _e('Password','osCommerce'); ?></label>
                        <input type="text" name="vchPassword" id="vchPassword" value="<?=stripslashes($shop->vchPassword);?>" class="text">
                    </div>
                    <div class="row">
                        <label for="vchDbName"><font color="red">*</font> <?php _e('DB Name','osCommerce'); ?></label>
                        <input type="text" name="vchDbName" id="vchDbName" value="<?=stripslashes($shop->vchDbName);?>" class="text">
                    </div>
                    <div class="row">
                        <label for="vchHost"><font color="red">*</font> <?php _e('Host','osCommerce'); ?></label>
                        <input type="text" name="vchHost" id="vchHost" value="<?=stripslashes($shop->vchHost);?>" class="text">
                    </div>
                    <p class="submit">
                        <input type="submit" name="submit" value="<?php _e('Update Shop','osCommerce'); ?> &raquo;">
                    </p>
                    <input type="hidden" name="form_name" id="form_name" value="frm_osc_edit">
