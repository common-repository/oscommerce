		...
		function osc_list_categories($db, $shop_id, $sel_shop_id, $sel_cat_id, $level)
		{
			...
			if($db->num_rows > 0)
			{
				...
				{	
					...
					echo '<li class="cat-item cat-item-1"><a name="'. $res_cats[$j]->categories_name .'_'. $shop_id .'"></a><a href="'. wp_nonce_url('products?shopID='. $shop_id .'&amp;catID='. $res_cats[$j]->categories_id .'#'. $res_cats[$j]->categories_name .'_'. $shop_id) .'" title="'. attribute_escape($res_cats[$j]->categories_name .' p('. $osc_products->record_count .')') .'">'. $res_cats[$j]->categories_name .' p('. $osc_products->record_count .')' .'</a>';
