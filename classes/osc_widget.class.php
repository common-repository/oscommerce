		...
		function display($args)
		{
			...
			if($this->num_rows > 0)
			{
				...
				for($i = 0 ; $i < count($res_arr) ; $i++)
				{
					...
					if($osc_products->record_count > 0)
					{
						$osc_categories->osc_count_categories($shop_db);
						
						if($osc_categories->record_count > 0)
						{
							echo '<li class="widget widget_categories"><h3><a name="'. $res_arr[$i]->vchShopName .'"></a><a href="'. wp_nonce_url('?shopID='. $intID .'#'. $res_arr[$i]->vchShopName) .'" title="'. attribute_escape($res_arr[$i]->vchShopName .'&nbsp;c('. $osc_categories->record_count .')&nbsp;p('. $osc_products->record_count .')') .'">'. $res_arr[$i]->vchShopName .'&nbsp;c('. $osc_categories->record_count .')&nbsp;p('. $osc_products->record_count .')</a></h3>';
